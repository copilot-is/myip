// https://github.com/metowolf/qqwry.ipdb/blob/master/src/format/index.js
...
const ispfix = require('./data/ispfix');
...
module.exports = (country, area) => {
   ...
   */
  const isps =
    isp_list[`${result.country_name}-${result.region_name}`] ||
    isp_list[result.country_name] ||
    [];
  for (const isp of isps) {
    if (area.includes(isp)) {
      result.isp_domain = ispfix[isp] ? ispfix[isp] : isp;
      break;
    }
  }
   ...
   */
  let owners =
    owner_list[`${result.country_name}-${result.region_name}`] ||
   ...
   * GEO and Postal
   */
  if (geo[result.country_name][result.region_name][result.city_name]) {
    result.latitude =
      geo[result.country_name][result.region_name][result.city_name].latitude;
    result.longitude =
      geo[result.country_name][result.region_name][result.city_name].longitude;
    result.postal =
      geo[result.country_name][result.region_name][result.city_name].postal;
  }
